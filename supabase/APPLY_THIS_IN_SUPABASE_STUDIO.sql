...
--      Migrated rows keep CURRENT behavior (assign on lesson complete). ────
do $$ begin
  if exists (
...
--      task_template + snapshot current title/description. ───────────────
do $$ begin
  if exists (
    select 1 from information_schema.tables
    where table_schema = 'public' and table_name = 'lesson_task_templates'
...
-- ── 5. task_assignment_rules — admin-mission targeting (flexible jsonb). ──
...
-- ── 6. task_event_logs — assignment + lifecycle audit trail. ─────────────
