    ...
    <div className="w-[280px] max-w-full">
      ...
      <div className="relative">
        <select
          id="plan-select"
          defaultValue="pro"
          className="w-full h-11 pl-3.5 pr-10 rounded-[12px] bg-white border border-ink-100 text-[13.5px] text-ink-900 appearance-none outline-none focus:border-rose-300 focus:ring-2 focus:ring-rose-100 transition-colors"
        >
...
export function FileDropzone() {
  return (
    <div className="w-[360px] max-w-full">
      <div className="rounded-[14px] border-2 border-dashed border-ink-200 bg-cream-50 hover:border-rose-300 hover:bg-rose-50/40 transition-colors px-6 py-8 flex flex-col items-center text-center cursor-pointer">
      ...
      {all.map((c) => {
        ...
        return (
          <button
            ...
            aria-pressed={on}
            className={
              "inline-flex items-center gap-1.5 h-8 px-3 rounded-full text-[12.5px] font-medium border transition-colors " +
              (on
                ? "bg-rose-600 border-rose-600 text-white"
                : "bg-white border-ink-100 text-ink-500 hover:border-ink-300 hover:text-ink-900")
            }
          >
