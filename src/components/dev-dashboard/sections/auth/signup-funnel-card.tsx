...
import { SIGNUP_FUNNEL } from "@/lib/dev-dashboard/mock-data";
import type { SignupFunnelStage } from "@/lib/dev-dashboard/types";

export function SignupFunnelCard({ data }: { data?: SignupFunnelStage[] }) {
  const stages = data ?? SIGNUP_FUNNEL;
  const top = stages[0]?.count ?? 0;

  return (
    <DevSectionCard title="Signup Funnel">
          ...
          return (
            <li key={stage.key} className="flex items-center gap-3 text-[12.5px]">
              <div className="flex-1 flex justify-center">
                <div
                  className="relative h-9 inline-flex items-center justify-center rounded-[6px] bg-[var(--dev-accent-soft)] border border-[var(--dev-accent-border)] text-[var(--dev-accent-text)] text-[12.5px] font-medium px-3 truncate"
                  style={{ width: `${widthPercent}%`, minWidth: "32%" }}
                  aria-label={`${stage.label}: ${stage.count} users${
                    stage.dropOffPercent != null ? `, ${stage.dropOffPercent}% drop-off` : ""
                  }`}
                >
                  {stage.label}
                </div>
