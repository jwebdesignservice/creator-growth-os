...
import { AUTH_METRIC_CARDS } from "@/lib/dev-dashboard/mock-data";
import type { AuthMetricCard, AuthMetricTone } from "@/lib/dev-dashboard/types";
import { cn } from "@/lib/cn";

const TONE_COLOR: Record<AuthMetricTone, string> = {
...
const ICON_TILE: Record<AuthMetricTone, string> = {
...
export function AuthMetricCards({ data }: { data?: AuthMetricCard[] }) {
  const cards = data ?? AUTH_METRIC_CARDS;
  return (
    <section
...
function Tile({ metric }: { metric: AuthMetricCard }) {
    ...
    <div className="dev-card p-4 flex flex-col gap-2.5 min-h-[156px]">
      ...
      <div className="-mx-1 mt-auto">
        <DevSparkline
          data={metric.series}
          color={color}
          gradientId={`auth-spark-${metric.key}`}
          height={44}
        />
      </div>
    </div>
