...
import { USER_GROWTH_CHART } from "@/lib/dev-dashboard/mock-data";

const CHART_W = 700;
const CHART_H = 200;

export function UserGrowthTrendsCard() {
  const { series, xLabels, yLabels, yMax } = USER_GROWTH_CHART;

  // Build all series paths against a shared y-max so the lines stay comparable.
  ...
  return (
    <DevSectionCard
      title="User Growth & Activity Trends"
      trailing={
        <div className="flex flex-wrap items-center gap-2">
          <CardSelect label="Metric"      value="Users" />
          <CardSelect label="Granularity" value="Daily" />
          <Link
            href="/dev/analytics"
    ...
    >
      ...
      <ul className="flex flex-wrap items-center gap-4 mb-3">
        {series.map((s) => (
          <li key={s.key} className="inline-flex items-center gap-1.5 text-[12px] text-[var(--dev-text-secondary)]">
...
/** Build an SVG path against an explicit y-max so multiple series remain
 *  comparable on the same chart axis. Mirrors `sparklinePath` but uses
 *  a shared scale instead of each series' own min/max. */
function pathAgainstMax(values: number[], yMax: number, w: number, h: number, padY = 0): string {
  if (values.length === 0) return "";
  const stepX = values.length > 1 ? w / (values.length - 1) : 0;
  const inner = h - padY * 2;

  // Re-use sparklinePath when uniform, otherwise compute manually.
