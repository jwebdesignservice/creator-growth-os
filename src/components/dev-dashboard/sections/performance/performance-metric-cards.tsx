...
import type { PerfMetricCard, PerfMetricTone } from "@/lib/dev-dashboard/types";
import { cn } from "@/lib/cn";

const TONE_COLOR: Record<PerfMetricTone, string> = {
...
const ICON_TILE: Record<PerfMetricTone, string> = {
...
export function PerformanceMetricCards({ metrics }: { metrics: PerfMetricCard[] }) {
  return (
    <section
...
function Tile({ metric }: { metric: PerfMetricCard }) {
    ...
    <div className="dev-card p-4 flex flex-col gap-2.5 min-h-[156px]">
      ...
      <div className="-mx-1 mt-auto">
        <DevSparkline
          data={metric.series}
          color={color}
          gradientId={`perf-spark-${metric.key}`}
          height={44}
        />
      </div>
    </div>
