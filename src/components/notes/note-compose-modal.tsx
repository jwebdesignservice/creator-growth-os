...
import { RichNoteEditor, type RichNoteEditorHandle } from "@/components/notes/rich-note";
import {
  noteIsEmpty,
  noteTextLength,
  stripUnsafeNoteHtml,
...
/* Labeled scaffolds a creator commonly jots — one click drops the prefix. */
...
/**
 * Composer popup for a free-standing program note (NOT tied to a lesson/video).
 * Mirrors the lesson "Create note" composer — title heading, rich editor,
 * quick-add scaffolds — opened by the program board's "+ New note" tile.
 */
export function NewNoteModal({
  ...
  function save() {
    ...
    // Title becomes the note's heading (first block) — the cards + reader
    // already style a note's first block as the title, so no schema change.
    const esc = (s: string) =>
  ...
  return createPortal(
    ...
    >
      <div
        className="bg-white rounded-[18px] shadow-xl border border-ink-100 w-full max-w-[560px] p-6 max-h-[90vh] overflow-y-auto"
        ...
        {saved ? (
          <div className="rounded-[14px] bg-emerald-50 border border-emerald-200 px-4 py-6 text-center">
            ...
            <p className="text-[12.5px] text-ink-500 mt-0.5">
              It&apos;s now on your Notes board.
            </p>
          </div>
        ) : (
          <>
            {/* Attached-to context — this program (not a specific video) */}
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="inline-flex items-center gap-1.5 h-7 pl-2 pr-2.5 rounded-full bg-cream-100 border border-ink-200 text-[12px] font-semibold text-ink-700 min-w-0">
                ...
                <span className="truncate">This program</span>
              </span>
            </div>
